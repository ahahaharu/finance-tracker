...
Учётные записи заводятся с паролем `demo-tracker-2026`.

Курсы за период дописываются только на те даты, которых в базе нет: если перед
наполнением выполнялась `pnpm rates:refresh`, курсы Нацбанка остаются, а пробелы
закрываются сгенерированным рядом. Операции считаются по тому же ряду, поэтому
`rate` каждой записи совпадает с курсом её `rateDate` в таблице `ExchangeRate`.

Сценарий идемпотентен: повторный запуск удаляет обе демонстрационные учётные
записи вместе со связанными данными и создаёт их заново. Курсы при этом
сохраняются — они ничьей учётной записи не принадлежат.
